describe("Block Store", function(){
  ...
  it("should save data if no data is given", function(){
  ...
  it("should store the blocks state when the save method is invoked on the block", function(){
    editor.blocks[0].$$('.st-text-block').html("Hello, this is a test");

    editor.blocks[0].save();

    expect(editor.blocks[0].getData().text).toBe("Hello, this is a test");
  });
});